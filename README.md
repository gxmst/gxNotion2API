...
如果账号池全部为 `auto_only`，`/v1/models` 只返回 `auto`，这是实际能力的反映。NextChat、Cherry Studio 等客户端应选择或自定义填写 `auto`，再由工作区所有者在此面板手动限制候选模型；不要将 `opus-5` 等名称当成试用区的请求级选模权限。回答中的实际模型仍以上游报告为准。
...
注意单个附件的 20 MiB 上限只在附件本身被单独校验时生效。内联附件是随请求体一起发送的 Base64，因此实际能传多大取决于 `limits.max_request_body_bytes`（默认 4 MiB）。Base64 会让数据膨胀约 4/3，再减去 JSON 外壳，默认配置下单文件大约 3 MiB 就会触顶。需要更大的内联附件时，请同时调大 `limits.max_request_body_bytes`；管理台会按该上限在本地提前拦截并说明原因，不会等到服务端返回一个笼统的失败。
...
部分正向 HTTP 代理默认只允许向 443 端口发起 CONNECT，HTTP 附件的 80 端口可能被拒绝。此时优先使用源站支持的 HTTPS 附件链接，或由代理管理员放行目标端口的 CONNECT。服务会返回端口限制的提示，不会改为直连或让代理重新解析目标域名。
